...
def indicator_function(norm, k):
    if k == 'intimate':
        return 1 if norm < 0.45 else 0
    elif k == 'personal':
        return 1 if 0.45 <= norm < 1.2 else 0
    elif k == 'social':
        return 1 if 1.2 <= norm < 3.6 else 0
    elif k == 'public':
        return 1 if norm >= 3.6 else 0
    else:
        return 0
...
def total_time(agents, robot):
    ...
    dur = (t2 - t1).to_sec()
    rospy.loginfo('Time_to_reach_goal computed: %.2f secs' % dur)
    return [dur]
...
def robot_path_length(agents, robot):
    ...
    rospy.loginfo('Path_length computed: %.2f m' % path_length)
    return [path_length]
...
def cumulative_heading_changes(agents, robot):
    ...
    rospy.loginfo('Cumulative_heading_changes: %.2f rads' % chc)
    return [chc, chc_list]
...
def avg_closest_person(agents, robot):
    ...
    rospy.loginfo('Average_closest_person: %.2f m' % avg_dist)
...
def minimum_distance_to_people(agents, robot):
    ...
    rospy.loginfo('Minimum_distance_to_people: %.2f m' % min_dist)
    return [min_dist]
...
def maximum_distance_to_people(agents, robot):
    ...
    rospy.loginfo('Maximum_distance_to_people: %.2f m' % max_dist)
    return [max_dist]
...
def group_intimate_space_intrusions(agents, robot):
    ...
    rospy.loginfo('Group_intimate_space_intrusions: %.2f %% of the total time' % r[0])
    return r

def group_personal_space_intrusions(agents, robot):
    r = group_space_intrusions(agents, robot, 'personal')
    rospy.loginfo('Group_personal_space_intrusions: %.2f %% of the total time' % r[0])
    return r

def group_social_space_intrusions(agents, robot):
    r = group_space_intrusions(agents, robot, 'social')
    rospy.loginfo('Group_social_space_intrusions: %.2f %% of the total time' % r[0])
    return r
...
def avg_acceleration(agents, robot):
    ...
    for i in range(len(robot) - 1):
        ...
        tf = rospy.Time.from_sec(agents[i+1].header.stamp.to_sec())
        ti = rospy.Time.from_sec(agents[i].header.stamp.to_sec())
        dt = (tf - ti).to_sec()
        if dt != 0.0:
            accel = dv / dt
...
def social_force_on_robot(agents, robot):
    ...
    sf = 0.0
    sf_list = []
    for rb, agts in zip(robot, agents):
        f = sfm.modulusSocialForce2(rb, agts)
        sf += f
        sf_list.append(f)
    return [sf, sf_list]
